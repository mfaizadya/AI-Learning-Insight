...
const TopNav = () => {
  ...
  const mobileMenuItems = [
    ...
    { label: "Pretest", href: "/dashboard/pretest" },
    { label: "Akun Saya", href: "/dashboard/account" },
  ];
  ...
  return (
    <>
      ...
      {isMobileMenuOpen && (
        <div
          className="fixed inset-0 bg-black/20 backdrop-blur-sm z-30 transition-opacity duration-300 lg:hidden"
      ...
      <nav className="w-full px-4 md:px-8 py-4 md:py-6 flex justify-between items-center bg-gradient-to-b from-[#F5F5F5] to-transparent sticky top-0 z-40 transition-all duration-300">
        <div
          ...
          className={`relative w-full flex justify-between items-center py-3 md:py-4 rounded-3xl px-4 md:px-6 transition-all duration-300 ${
            isScrolled
        ...
        >
          ...
          <div
            className={`
              absolute top-full left-0 w-full mt-2 bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden transition-all duration-300 ease-in-out origin-top lg:hidden
              ${
                isMobileMenuOpen
              ...
              {mobileMenuItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    onClick={() => setIsMobileMenuOpen(false)}
                    className={`
                      px-6 py-4 rounded-2xl text-base font-bold text-center transition-colors
                      ${
                        isActive
          ...
          <section className="flex items-center gap-3">
            ...
            <div className="lg:hidden">
              <button
                ...
                className="p-2 -ml-1 text-gray-600 hover:bg-gray-100 rounded-full transition-colors active:scale-95"
              >
                {isMobileMenuOpen ? <X size={24} /> : <Logs size={24} />}
              </button>
            </div>

            {/* breadcrump */}
            <h1 className="px-4 sm:px-4 md:px-6 py-1.5 md:py-2 rounded-full shadow-none bg-[#EEF2FF] text-primary font-bold text-[0.70rem] sm:text-xs md:text-sm border border-none transition-all truncate max-w-[150px] md:max-w-none">
              {pageTitle}
            </h1>
          </section>
